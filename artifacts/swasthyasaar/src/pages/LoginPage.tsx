...
import { Stethoscope, BarChart2, Settings, UserCircle } from "lucide-react";
    ...
    <div className="min-h-screen bg-background flex flex-col fade-in">
      <div className="flex-1 flex flex-col items-center justify-center p-4">
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground">Demo Mode — ArogyaSaathi MVP</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-6xl w-full">
          {/* Patient Role */}
          <Link href="/patient/0">
            <Card className="p-8 flex flex-col items-center justify-center text-center hover-lift cursor-pointer border-2 border-transparent hover:border-[#1A1A2E] transition-colors h-64 group bg-card">
              <div className="w-16 h-16 bg-slate-100 dark:bg-slate-800 rounded-full flex items-center justify-center mb-4 group-hover:bg-slate-200 transition-colors">
                <UserCircle className="w-8 h-8 text-slate-600 dark:text-slate-300" />
              </div>
              <h2 className="text-xl font-bold text-foreground mb-2">Patient</h2>
              <p className="text-sm text-muted-foreground">View your care pathway, upcoming visits, and health history.</p>
            </Card>
          </Link>
